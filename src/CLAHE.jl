...
function map_histogram(edges::AbstractArray, counts::AbstractVector{T}, minval::Union{Real,AbstractGray}, maxval::Union{Real,AbstractGray}) where T<:Integer
    n_pixels = sum(counts)
...
function (f::ContrastLimitedAdaptiveHistogramEqualization)(out::AbstractArray{<:Color3}, img::AbstractArray{<:Color3})
    T = eltype(img)
    yiq = convert.(YIQ, img)
